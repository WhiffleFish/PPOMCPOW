function simulate(pomcp::TreeParallelPOWPlanner, h_node::POWTreeObsNode, s, d::Int, rng::AbstractRNG)
    ...
    if tree.n_a_children[best_node][] <= sol.k_observation*(tree.n[best_node][]^sol.alpha_observation)
        ...
        sp, o, r = @gen(:sp, :o, :r)(pomcp.problem, s, a, rng)
        ...
        sp, r = @gen(:sp, :r)(pomcp.problem, s, a, rng)
    ...
    if new_node
        R = r + γ*estimate_value(pomcp.solved_estimate, pomcp.problem, sp, POWTreeObsNode(tree, hao), d-1, rng)
    else
        ...
        R = r + γ*simulate(pomcp, POWTreeObsNode(tree, hao), sp, d-1, rng)
    end
